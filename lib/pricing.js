...
function loadCache() {
  try {
    ...
    // Validate cache: reject if it contains poisoned pricing (negative values)
    if (raw.pricing && raw.pricing['openrouter/auto']) {
      const autoPrice = raw.pricing['openrouter/auto'];
      if (autoPrice.input < 0 || autoPrice.output < 0) {
        console.log('[Pricing] Cache contains poisoned openrouter/auto pricing, deleting');
