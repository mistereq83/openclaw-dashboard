...
function start(stateDir, agentIds) {
  ...
  function scheduleNext() {
    ...
    timer = setTimeout(async () => {
      ...
      console.log(`[Scheduler] Starting daily analysis for ${dateStr}`);
      isRunning = true;
      ...
      try {
        ...
        console.log(`[Scheduler] Analysis complete for ${dateStr}: ${report.totals.sessions} sessions`);
      } catch (err) {
        lastRun = { date: dateStr, at: new Date().toISOString(), success: false, error: err.message };
        console.error(`[Scheduler] Analysis failed for ${dateStr}:`, err.message);
      }
