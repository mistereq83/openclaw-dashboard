...
function init() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS daily_token_stats (
      ...
      reconciled_cost REAL DEFAULT NULL,  -- from OpenRouter Activity API
    ...
    CREATE INDEX IF NOT EXISTS idx_token_stats_date ON daily_token_stats(date);
    CREATE INDEX IF NOT EXISTS idx_token_stats_agent ON daily_token_stats(agent_id);
    CREATE INDEX IF NOT EXISTS idx_token_stats_model ON daily_token_stats(model);
    CREATE INDEX IF NOT EXISTS idx_token_stats_composite ON daily_token_stats(agent_id, date, model);
  `);
}
  ...
  const insertMany = d.transaction((msgs) => {
    ...
    for (const msg of msgs) {
      const info = insert.run(
        ...
        msg.timestamp || null,
        msg.tokens || null,
        // Sanitize: cost must be a small positive number (dollars, not tokens)
        // OpenClaw bug writes token counts as cost — reject anything > $100/msg
        (msg.cost && msg.cost > 0 && msg.cost < 100) ? msg.cost : 0,
...
function upsertDailyTokenStats(date, agentId, agentName, model, stats) {
  const stmt = getDb().prepare(`
    INSERT INTO daily_token_stats (
      date, agent_id, agent_name, model, 
  ...
  `);
  stmt.run(
    date, agentId, agentName, model || 'unknown',
    stats.inputTokens || 0,
    ...
    // Guard: reject negative or absurd costs (> $1000/agent/day/model = certainly token count bug)
    (stats.estimatedCost > 0 && stats.estimatedCost < 1000) ? stats.estimatedCost : 0
  );
}

function getDailyTokenStats(date, agentId, model) {
  const d = getDb();
  if (agentId && model) {
    return d.prepare('SELECT * FROM daily_token_stats WHERE date = ? AND agent_id = ? AND model = ?').get(date, agentId, model);
  } else if (agentId) {
    return d.prepare('SELECT * FROM daily_token_stats WHERE date = ? AND agent_id = ?').all(date, agentId);
  } else {
    return d.prepare('SELECT * FROM daily_token_stats WHERE date = ?').all(date);
  }
}

function getTokenStatsByAgent(agentId, dateFrom, dateTo) {
  const d = getDb();
  return d.prepare(`
    SELECT date, model, 
      input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, 
      total_tokens, message_count, estimated_cost, reconciled_cost
    FROM daily_token_stats 
    WHERE agent_id = ? AND date >= ? AND date <= ?
    ORDER BY date DESC, model
  `).all(agentId, dateFrom, dateTo);
}

function getTokenStatsSummary(yearMonth, agentId) {
  const d = getDb();
  const like = yearMonth + '%'; // e.g. '2026-03%'
  ...
  if (agentId) {
    return d.prepare(`
      SELECT agent_id, agent_name, model,
        ...
        SUM(reconciled_cost) as reconciled_cost,
        COUNT(*) as active_days
      FROM daily_token_stats WHERE date LIKE ? AND agent_id = ?
  ...
  return d.prepare(`
    ...
    FROM daily_token_stats WHERE date LIKE ?
...
function migrateFixBrokenCosts() {
  const d = getDb();
  const marker = d.prepare(`SELECT 1 FROM daily_stats WHERE date = 'migration-v3'`).get();
  if (marker) return 0; // Already migrated

  console.log('[DB] Running HARD cost migration v3 — dropping and recreating tables...');

  // Nuclear option: DROP + recreate (guarantees no stale data survives)
  d.exec(`DROP TABLE IF EXISTS messages`);
  d.exec(`DROP TABLE IF EXISTS daily_stats`);

  // Re-init creates fresh tables
  d.exec(`
    CREATE TABLE IF NOT EXISTS daily_stats (
      date TEXT NOT NULL,
      agent_id TEXT NOT NULL,
  ...
  // Mark migration
  d.prepare(`INSERT INTO daily_stats (date, agent_id, agent_name, sessions, messages, user_messages, cost) VALUES ('migration-v3', '_system', 'migration', 0, 0, 0, 0)`).run();

  console.log('[DB] Migration v3 complete — tables recreated from scratch');

  console.log('[DB] Migration complete — archive-worker + backfill will rebuild data');
  return r1.changes + r2.changes;
}

function getTokenStatsSummaryRange(dateFrom, dateTo, agentId) {
  const d = getDb();
  ...
  if (agentId) {
    return d.prepare(`
      SELECT agent_id, agent_name,
        ...
        SUM(reconciled_cost) as reconciled_cost,
        COUNT(DISTINCT date) as active_days
      FROM daily_token_stats WHERE date >= ? AND date <= ? AND agent_id = ?
      GROUP BY agent_id, agent_name
      ORDER BY total_tokens DESC
    `).all(dateFrom, dateTo, agentId);
  }
  ...
  return d.prepare(`
    SELECT agent_id, agent_name,
      ...
      SUM(reconciled_cost) as reconciled_cost,
      COUNT(DISTINCT date) as active_days
    FROM daily_token_stats WHERE date >= ? AND date <= ?
    GROUP BY agent_id, agent_name
    ORDER BY total_tokens DESC
  `).all(dateFrom, dateTo);
}
...
module.exports = {
  ...
  // Token stats
  upsertDailyTokenStats,
  getDailyTokenStats,
  getTokenStatsByAgent,
  getTokenStatsSummary,
  getTokenStatsSummaryRange,
};
