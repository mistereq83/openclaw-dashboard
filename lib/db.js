...
function migrateFixBrokenCosts() {
  const d = getDb();
  const marker = d.prepare(`SELECT 1 FROM daily_stats WHERE date = 'migration-v2'`).get();
  if (marker) return 0; // Already migrated

  console.log('[DB] Running one-time cost migration v2 (clearing ALL poisoned data)...');
  ...
  d.prepare(`INSERT INTO daily_stats (date, agent_id, agent_name, sessions, messages, user_messages, cost) VALUES ('migration-v2', '_system', 'migration', 0, 0, 0, 0)`).run();
