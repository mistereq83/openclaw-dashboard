...
// Fix negative costs in all tables (caused by OpenClaw bug with openrouter)
function fixNegativeCosts() {
  const d = getDb();
  let totalFixed = 0;

  const r1 = d.prepare(`UPDATE messages SET cost = ABS(cost) WHERE cost < 0`).run();
  if (r1.changes > 0) {
    console.log(`[DB] Fixed ${r1.changes} messages with negative costs`);
    totalFixed += r1.changes;
  }

  const r2 = d.prepare(`UPDATE daily_stats SET cost = ABS(cost) WHERE cost < 0`).run();
  if (r2.changes > 0) {
    console.log(`[DB] Fixed ${r2.changes} daily_stats rows with negative costs`);
