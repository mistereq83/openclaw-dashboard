      ...
      for (const file of files) {
        ...
        for (const line of content.split('\n')) {
          ...
          try {
            ...
            const rawCost = effectiveUsage.cost && typeof effectiveUsage.cost === 'object' 
              ? effectiveUsage.cost.total : null;
            if (rawCost && rawCost > 0 && rawCost < 10) {
              stats.estimatedCost += rawCost;
            } else {
              const { estimateCostFromTokens } = require('./parser');
              const est = estimateCostFromTokens(effectiveUsage, model);
              // Final sanity: estimated cost per message max $10
              if (est > 0 && est < 10) {
