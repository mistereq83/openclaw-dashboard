...
const { listSessions, getSessionMessages, getAgentDisplayName, getSessionCostByDate } = require('./parser');
const db = require('./db');
...
function computeAndStoreDailyStats(stateDir, agentIds, dateStr) {
  ...
  for (const agentId of agentIds) {
    ...
    for (const session of daySessions) {
      ...
      // Koszty: sumuj TYLKO te z tego dnia (fix double-counting dla sesji wielodniowych)
      const sessionCosts = getSessionCostByDate(stateDir, agentId, session.id);
      for (const c of sessionCosts) {
        if (!c.timestamp) continue;
        const costDate = new Date(c.timestamp);
        if (costDate >= dayStart && costDate <= dayEnd) {
          totalCost += c.cost;
        }
      }
    }
