      ...
      for (const file of files) {
        ...
        for (const line of content.split('\n')) {
          ...
          try {
            ...
            if (effectiveUsage.cost && effectiveUsage.cost.total > 0) {
              stats.estimatedCost += effectiveUsage.cost.total;
            } else {
              const { estimateCostFromTokens } = require('./parser');
              stats.estimatedCost += estimateCostFromTokens(effectiveUsage, model);
            }
