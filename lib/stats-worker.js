...
function computeAndStoreDailyStats(stateDir, agentIds, dateStr) {
  ...
  for (const agentId of agentIds) {
    ...
    for (const session of daySessions) {
      ...
      // Cost: assign full session cost to the day of lastMessage ONLY
      // This prevents double-counting for multi-day sessions
      const sessionLastDay = new Date(session.lastMessage).toISOString().split('T')[0];
      if (sessionLastDay === dateStr) {
      ...
      for (const file of files) {
        ...
        for (const line of content.split('\n')) {
          ...
          try {
            ...
            // Skip if not a message with usage or not from target date
            if (obj.type !== 'message' || !obj.usage || !obj.timestamp) continue;
            
            const msgDate = new Date(obj.timestamp);
            if (msgDate < dayStart || msgDate > dayEnd) continue;
            
            const usage = obj.usage;
            const model = (obj.message && obj.message.model) || 'unknown';
            ...
            stats.totalTokens += (usage.input || 0) + (usage.output || 0) + (usage.cacheRead || 0) + (usage.cacheWrite || 0);
            stats.messageCount += 1;
            
            // Use existing cost if available, or estimate from tokens
            if (usage.cost && usage.cost.total > 0) {
              stats.estimatedCost += usage.cost.total;
            } else {
              // Use estimateCostFromTokens from parser.js
              const { estimateCostFromTokens } = require('./parser');
              stats.estimatedCost += estimateCostFromTokens(usage, model);
            }
