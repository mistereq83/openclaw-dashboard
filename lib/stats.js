...
function getAgentStats(stateDir, agentId, month) {
  ...
  // Falls back to live JSONL sum if SQLite has no data yet
  const allTimeFromDb = db.getAllTimeCost(agentId);
  const allTimeCost = allTimeFromDb || sessions.reduce((sum, s) => sum + (s.cost || 0), 0);
