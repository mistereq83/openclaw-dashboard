...
function getAgentStats(stateDir, agentId, month) {
  ...
  // All-time cost: use the HIGHER of SQLite vs live JSONL
  // SQLite survives JSONL deletion, JSONL is more accurate for current sessions
  const allTimeFromDb = db.getAllTimeCost(agentId) || 0;
  const allTimeFromFiles = sessions.reduce((sum, s) => sum + (s.cost || 0), 0);
  const allTimeCost = Math.max(allTimeFromDb, allTimeFromFiles);
  ...
  todayCost = allSessions.reduce((sum, s) => (
    s.lastMessage >= todayStart ? sum + (s.cost || 0) : sum
  ), 0);
...
function getOverviewStats(stateDir, agentIds) {
  ...
  for (const agentId of agentIds) {
    ...
    totalTodayCost += stats.todayCost || 0;
    todayCostPerAgent.push({
      agentId,
      name: getAgentDisplayName(agentId),
      todayCost: Math.round((stats.todayCost || 0) * 10000) / 10000,
    });
  ...
  return {
    ...
    totalTodayCost: Math.round(totalTodayCost * 10000) / 10000,
    todayCostPerAgent,
    topDays,
    last7DaysLabels,
    last7DaysActivity,
...
function getAgentActivityVisuals(stateDir, agentId, month) {
...
module.exports = { getAgentStats, getOverviewStats, generateCsvExport, getAgentActivityVisuals };
