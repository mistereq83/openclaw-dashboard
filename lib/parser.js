...
function lookupPricing(model) {
  ...
  // For 'auto' (openrouter/auto), 'unknown', or any unrecognized model — use default
  // This ensures agents using openrouter/auto always get an estimated cost
  if (normalized === 'auto' || normalized === 'unknown' || !normalized) return DEFAULT_PRICING;

  return DEFAULT_PRICING; // Final fallback: always return something rather than null
}
        ...
        if (content) {
          messages.push({
            ...
            cost: (() => {
              ...
              let c = 0;
              if (rawCost > 0 && rawCost < 10) {
                c = rawCost; // Valid cost
              } else {
                c = estimateCostFromTokens(u, msg.model || obj.model);
              }
              // Sanity cap: single message max $10
              return (c > 0 && c < 10) ? c : 0;
            })(),
  ...
  for (const line of content.split('\n')) {
    ...
    try {
      ...
      if (obj.type === 'message' && usage) {
        ...
        if (rawCost > 0 && rawCost < 10) {
          // Valid cost (max $10/message is reasonable, anything higher = bug)
          msgCost = rawCost;
        } else {
          // Estimate from tokens + pricing table
          msgCost = estimateCostFromTokens(usage, model);
        }

        // Final sanity: single message can't cost more than $10
        if (msgCost > 0 && msgCost < 10) {
          total += msgCost;
        }
      }
    } catch { /* skip */ }
