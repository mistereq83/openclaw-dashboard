...
function estimateCostFromTokens(usage, model) {
  ...
  if (!pricing) return 0;
  const input = (usage.input || 0) / 1e6 * pricing.input;
  const output = (usage.output || 0) / 1e6 * pricing.output;
  const cacheRead = (usage.cacheRead || 0) / 1e6 * pricing.cacheRead;
  const cacheWrite = (usage.cacheWrite || 0) / 1e6 * pricing.cacheWrite;
        ...
        if (content) {
          messages.push({
            ...
            cost: (() => {
              ...
              // Fallback: estimate from tokens + model
              return estimateCostFromTokens(u, msg.model);
            })(),
    ...
    try {
      ...
      if (obj.type === 'message' && usage) {
        ...
        if (rawCost > 0) {
          ...
          total += estimateCostFromTokens(usage, model);  // normalizeModel handled inside
        }
      }
    } catch { /* skip */ }
  }
