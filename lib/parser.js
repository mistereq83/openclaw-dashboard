      ...
      if (obj.type === 'message' && obj.message) {
        ...
        if (content) {
          messages.push({
            ...
            tokens: obj.usage ? obj.usage.totalTokens : null,
            cost: obj.usage && obj.usage.cost ? obj.usage.cost.total : 0,
...
function calcSessionCost(stateDir, agentId, sessionId) {
  ...
  for (const line of content.split('\n')) {
    ...
    try {
      ...
      if (obj.type === 'message' && obj.usage && obj.usage.cost) {
        total += obj.usage.cost.total || 0;
      }
    } catch { /* skip */ }
  }
  return total;
