...
const { analyzeSession, OLLAMA_MODEL } = require('./ollama');
...
async function runDailyAnalysis(stateDir, agentIds, dateStr, onProgress) {
  ...
  const report = {
    ...
    model: OLLAMA_MODEL,
    agents: [],
    totals: { sessions: 0, avgQuality: null, avgSentiment: null, issues: 0, escalations: 0 },
  };
  ...
  for (let ai = 0; ai < agentIds.length; ai++) {
    ...
    report.totals.issues += agentReport.issues.length;
